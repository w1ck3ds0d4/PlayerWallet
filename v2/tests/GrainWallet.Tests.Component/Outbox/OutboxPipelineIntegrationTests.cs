...
public sealed class OutboxPipelineIntegrationTests : IAsyncLifetime
{
    ...
    public async Task End_To_End_Pipeline_Delivers_Each_Event_Exactly_Once()
    {
        ...
        for (var i = 0; i < EventCount; i++)
        {
            ...
            var result = OperationResult.Success(balance, now);
            state.TrackOperation(operationId, result);

            var saved = await store.SaveAsync("integration-player", i, state, result, walletEvent);
            Assert.Equal(WalletStoreSaveStatus.Applied, saved.Status);
        ...
        Assert.Equal(
            Enumerable.Range(1, EventCount).Select(value => new Money(value, "EUR")),
            consumed.Select(walletEvent => walletEvent.BalanceAfter));
    ...
    private static async Task BootstrapSchemaAsync(string connectionString)
    {
        ...
        var sql = ReadEmbeddedResource(assembly, "GrainWallet.Api.Db.Schema.WalletStateAndOutbox.sql");
