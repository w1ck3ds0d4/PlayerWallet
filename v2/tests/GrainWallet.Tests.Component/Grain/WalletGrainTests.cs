...
public sealed class WalletGrainTests(WalletGrainTestCluster cluster)
{
    ...
    public async Task AddFunds_Creates_Wallet_With_Currency_Of_First_Add()
    {
        ...
        var result = await wallet.AddFundsAsync(Guid.NewGuid(), new Money(100m, "USD"));
        Assert.True(result.Succeeded);
        Assert.Equal(new Money(100m, "USD"), result.Balance);
    }
    ...
    [Fact]
    public async Task Same_OperationId_With_Different_Request_Is_Rejected()
    {
        var wallet = cluster.Wallet($"id-reuse-{Guid.NewGuid():N}");
        var operationId = Guid.NewGuid();
        await wallet.AddFundsAsync(operationId, new Money(100m, "EUR"));

        await Assert.ThrowsAnyAsync<Exception>(
            () => wallet.DeductFundsAsync(operationId, new Money(100m, "EUR")));
        Assert.Equal(new Money(100m, "EUR"), await wallet.GetBalanceAsync());
    }
    ...
    [Fact]
    public async Task Failed_Save_Does_Not_Leak_Into_Live_Balance_Or_Dedupe_Cache()
    {
        var playerId = $"failed-save-{Guid.NewGuid():N}";
        var operationId = Guid.NewGuid();
        var wallet = cluster.Wallet(playerId);
        cluster.Publisher.ShouldThrow = true;

        try
        {
            await Assert.ThrowsAnyAsync<Exception>(
                () => wallet.AddFundsAsync(operationId, new Money(25m, "EUR")));
            Assert.Equal(Money.Zero("EUR"), await wallet.GetBalanceAsync());
        }
        finally
        {
            cluster.Publisher.ShouldThrow = false;
        }

        var retried = await wallet.AddFundsAsync(operationId, new Money(25m, "EUR"));
        Assert.True(retried.Succeeded);
        Assert.Equal(new Money(25m, "EUR"), await wallet.GetBalanceAsync());
        Assert.Single(cluster.Publisher.Published, walletEvent => walletEvent.PlayerId == playerId);
    }
