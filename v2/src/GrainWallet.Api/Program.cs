...
using GrainWallet.Api.Endpoints;
...
if (usingPostgres && app.Environment.IsDevelopment())
{
    app.MapPost("/admin/reset-outbox", async (NpgsqlDataSource dataSource, ILogger<Program> logger, CancellationToken ct) =>
    {
        ...
        await using var command = new NpgsqlCommand(
            "DELETE FROM wallet_outbox WHERE published_at IS NOT NULL;",
            connection);
        command.CommandTimeout = 30;
        await command.ExecuteNonQueryAsync(ct);
        logger.LogInformation("Published wallet_outbox history removed by /admin/reset-outbox; pending rows retained.");
        return Results.Ok(new { reset = "published wallet_outbox rows", at = DateTimeOffset.UtcNow });
    })
    .WithTags("Admin")
    .WithSummary("Dev-only: remove published outbox history while retaining pending events.");
