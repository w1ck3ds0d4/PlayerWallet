...
/// Uses all-ISR acknowledgements and producer idempotence. The outbox remains at-least-once across the Kafka-ack/Postgres-mark boundary.
...
internal sealed class KafkaWalletEventPublisher : IWalletEventPublisher, IAsyncDisposable, IHealthCheck
{
        ...
        ILogger<KafkaWalletEventPublisher> logger)
    {
        ...
        // Keep broker retries ordered and duplicate-free. Cross-player batching in the drainer
        // retains throughput while each player has at most one application publish in flight.
