    ...
    ILogger<WalletOutboxDrainer> logger) : BackgroundService
{
    ...
    private static readonly string ClaimSql = $"""
        SELECT candidate.id, candidate.event_id, candidate.event_type, candidate.player_id, candidate.payload
        FROM (
        ...
        WHERE pg_try_advisory_xact_lock(hashtextextended(candidate.player_id, 0))
    ...
    private async Task<int> DrainBatchAsync(CancellationToken cancellationToken)
    {
        ...
        var publishTasks = batch
            .GroupBy(entry => entry.Event.PlayerId)
            .Select(group => PublishPlayerAsync(group, cancellationToken))
            .ToArray();
        var publishedByPlayer = await Task.WhenAll(publishTasks);
        publishSw.Stop();
        WalletMeters.DrainerPublishDuration.Record(publishSw.Elapsed.TotalMilliseconds);

        var publishedIds = publishedByPlayer.SelectMany(ids => ids).ToList();
    ...
    private async Task<IReadOnlyList<long>> PublishPlayerAsync(
        IEnumerable<DrainEntry> entries,
        CancellationToken cancellationToken)
    {
        var published = new List<long>();
        foreach (var entry in entries.OrderBy(item => item.Id))
        {
            if (!await PublishOneAsync(entry, cancellationToken))
            {
                break;
            }
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            if (walletEvent is null)
            {
                throw new InvalidDataException(
                    $"Unknown event type {eventType} in wallet_outbox id {id}; refusing to publish later events out of order.");
            }
